...
pub fn plugin(app: &mut App) {
    ...
    app.add_systems(OnEnter(Screen::Game), setup_leaderboard);
    app.add_systems(
        Update,
        (toggle_leaderboard, update_leaderboard).chain().run_if(in_state(Screen::Game)),
    );
}
...
fn update_leaderboard(
    ...
    for line in lines.iter().take(12) {
        ...
        out.push_str(&format!(
            ...
            line.total_mass,
            if line.is_mine { "  ← ты" } else { "" }
        ));
    }
    if lines.len() > 12 {
        out.push_str(&format!("\n…и ещё {} родов помельче", lines.len() - 12));
    }
