...
pub enum MutationRequest {
    ...
    /// Пересесть в самого развитого из своих потомков.
    ///
    /// Род — это не одно тело, а линия. Игрок должен уметь выбрать, каким её
    /// представителем играть, а не быть привязанным к тому, что досталось при
    /// рождении: вложился в колонию — играй лучшим, кто в ней вырос.
    TakeOverOffspring,
    /// Отдать своё тело боту и уйти в свободную камеру.
    ///
    /// Тело при этом не исчезает и не замирает: оно продолжает жить, есть и
    /// драться по тем же правилам, что и любой другой бот. Это и делает
    /// свободную камеру наблюдением, а не паузой.
    HandOverToBot,
